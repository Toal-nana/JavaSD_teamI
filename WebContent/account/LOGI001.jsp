...
<c:import url="/base.jsp">
  ...
  <c:param name="body">
    ...
    <%-- メインコンテンツ: ログインフォームを配置するコンテナ --%>
    <div class="col-lg-6 col-md-8 mx-auto my-3">

      <%-- カードコンポーネントでログインフォームを囲む --%>
      <div class="card shadow-sm border-light">
        ...
        <div class="card-body p-4">
          <form action="${pageContext.request.contextPath}/account/loginexecute" method="post">
            ...
            <%-- ユーザーID入力欄 (フローティングラベル付) --%>
            <div class="form-floating mb-3">
              <input type="text" name="id" id="id" class="form-control"
                     value="${id}"
                     placeholder="半角でご入力ください" maxlength="20"
                     style="ime-mode: disabled; background-color: #e6f2ff;" required>
              <label for="id">ID</label>
            </div>

            <%-- パスワード入力欄 (フローティングラベル付) --%>
            <div class="form-floating mb-3">
              <input type="password" name="password" id="password" class="form-control"
                     value="${password}"
                     placeholder="20文字以内の半角英数字でご入力ください" maxlength="20"
                     style="ime-mode: disabled; background-color: #e6f2ff;" required>
              <label for="password">パスワード</label>
            </div>

            <%-- パスワード表示切り替え用チェックボックス --%>
            ...
            <div class="text-center">
              <button type="submit" name="login"
                      class="btn btn-primary btn-lg fw-bold px-5">ログイン</button>
            </div>
          </form>
        </div>
      </div>
