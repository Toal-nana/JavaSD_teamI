	...
	<c:param name="body">
		<div class="container mt-2">
			...
			<form
				...
				<%-- 入学年度の表示 (変更不可) --%>
				<div class="mb-3">
					<label class="form-label">入学年度</label> <input type="text"
						name="entYear" value="${student.entYear}"
						class=" ms-3  form-control-plaintext" readonly>
				</div>
				...
				<div class="mb-3">
					<label class="form-label">氏名</label> <input type="text" name="name"
						value="${student.name}" class="form-control" required>
					<%-- 氏名のサーバーサイドエラーを表示 --%>
				...
				<div class="mb-4">
					<label class="form-label">クラス</label> <select name="classNum"
						...
						<c:forEach var="cls" items="${classList}">
							...
							<option value="${cls}"
								<c:if test="${cls == student.classNum}">selected</c:if>>${cls}</option>
						</c:forEach>
						<%-- クラスのサーバーサイドエラー表示 --%>
				...
				<div class="form-check mb-4">
					<input type="checkbox" class="form-check-input" id="isAttendCheck"
						name="isAttend" value="true"
						<c:if test="${student.attend}">checked</c:if>> <label
						class="form-check-label" for="isAttendCheck">在学中</label>
				</div>
